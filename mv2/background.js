function headersListener(details) {
	...
	const cspHeaderName = details.responseHeaders.filter(h => h.name.toLowerCase() === "content-security-policy-report-only").length > 0
		? "content-security-policy-report-only"
		: "content-security-policy"
	const csp = details.responseHeaders
		.filter(h => h.name.toLowerCase() === cspHeaderName)
		.map(h => h.value)[0];
	if (csp) {
		...
		return {
			...details,
			responseHeaders: [
				...details.responseHeaders.filter(h => h.name.toLowerCase() !== cspHeaderName),
				{ name: cspHeaderName, value: updatedCsp.map(pd => `${pd.directive} ${pd.value}`).join('; ') }
			]
		};
	}
